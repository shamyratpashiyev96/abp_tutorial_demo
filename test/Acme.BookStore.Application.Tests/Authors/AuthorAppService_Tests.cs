...
namespace Acme.BookStore.Authors
{
    ...
    {
        private readonly IAuthorAppService _authAppService;

        public AuthorAppService_Tests()
        {
            _authAppService = GetRequiredService<IAuthorAppService>();
        }

        [Fact]
        public async Task Should_Get_All_Authors_Without_Any_Filter()
        {
            var result = await _authAppService.GetListAsync(new GetAuthorListDto());
        ...
        {
            var result = await _authAppService.GetListAsync(
        ...
        {
            var authorDto = await _authAppService.CreateAsync(
                new CreateAuthorDto
                {
                    Name = "Edward Bellamy",
        ...
        public async Task Should_Not_Allow_To_Create_Duplicate_Author()
        {
            await Assert.ThrowsAsync<AuthorAlreadyExistsException>(async()=>{
                await _authAppService.CreateAsync(
                    new CreateAuthorDto
                    {
                        Name = "Douglas Adams",
